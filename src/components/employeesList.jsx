...
const EmployeesList = ({
  ...
  const handleDelete = (id, employeeId) => {
    // Set this row as deleting
    setDeletingRows((prev) => ({ ...prev, [id]: true }))

    // After animation completes, remove the item from the data
    setTimeout(() => {
      // Here you would call your actual delete API
      console.log(`Deleted row ${id}`)

      // Filter out the deleted employee from currentItems
      // You'll need to implement this part based on how your data is managed
      if (typeof data.setEmployees === "function") {
        data.setEmployees((prevEmployees) => prevEmployees.filter((emp) => emp._id !== employeeId))
      } else {
        // If we can't update the data source directly, at least mark this row as completely deleted
  ...
  const deleteAnimationKeyframes = `
    @keyframes deleteAnimation {
      ...
      80% { transform: translateX(30px) rotate(5deg); opacity: 0.5; }
      100% { transform: translateX(100px) translateY(-100px) rotate(15deg) scale(0.5); opacity: 0; }
    }
  `
        ...
        <table className="w-full">
          ...
          <tbody>
            {currentItems.map((employee, index) => {
              ...
              // Skip rendering if this row has been deleted and animation completed
                ...
                <>
                  <tr
                    ...
                    style={{
                      ...
                      animationDelay: deletingRows[indexOfFirstItem + index + 1] ? "0s" : `${index * 0.05}s`,
                      opacity: deletingRows[indexOfFirstItem + index + 1] ? 1 : 0,
                    }}
                  >
                    <td className="p-normal w-1/12 font-medium whitespace-nowrap transition-all duration-300">
                      {indexOfFirstItem + index + 1}
                    ...
                    <td className="p-normal w-2/12 whitespace-nowrap flex gap-3">
                      ...
                      <button
                        onClick={() => handleDelete(indexOfFirstItem + index + 1, employee._id)}
                        className="text-gray-600 hover:text-gray-800 transition-all duration-300"
                        style={{
                          transition: "all 0.3s ease",
                        }}
