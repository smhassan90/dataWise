...
const Sidebar = () => {
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-0 left-0 bg-secondary text-white transition-all duration-300 ease-in-out z-40
          ${
            sidebarOpen
              ? "lg:w-[16rem] lg:rounded-large"
              : "lg:w-[3rem] lg:rounded-large"
          }
          ${
            isMobileMenuOpen
              ? "w-[80%] max-w-[25rem] rounded-large h-full"
      ...
      >
        <div className={`h-full overflow-y-auto flex flex-col`}>
          <div className="flex items-center justify-between p-3">
            ...
            <div
              className={`w-full rounded-large shadow-md bg-white h-14 transition-all duration-300 ${
          ...
          <nav className="pt-0">
            <p
              className={`text-neutral-300 text-labelSize mb-4 px-4 transition-all duration-300 ${showHide}`}
            >
              OVERVIEW
            </p>
            <ul className="mt-2 flex flex-col">
              {sideBarMenu
                .filter((menu) =>
                  menu.level.includes(parseInt(level?.levelNumber))
                )
                .map((menu, index) => (
                  <li
                    key={index}
                    className="cursor-pointer hover:text-gray-300 flex justify-between"
                  >
                    <p
                      className={`w-full flex items-center py-3 px-4 ${
                        active === menu.title && sidebarOpen
                          ? "bg-white text-black space-x-6"
                          : "space-x-4"
                      }`}
                    ...
                    >
                      <motion.div
                        animate={
                          active === menu.title ? { scale: 1.2 } : { scale: 1 }
                        }
                    ...
                    <span
                      className={`${
                        active === menu.title ? "bg-orange-600 w-1" : ""
                      } rounded-l-lg`}
                    ></span>
                  </li>
                ))}
          ...
          <div className="border-t border-gray-700 mt-4 pt-4">
            <p
              className={`text-neutral-300 text-labelSize mb-2 px-4 transition-all duration-300 ${showHide}`}
            >
              OTHER
            </p>
            ...
            <ul className="mt-2 flex flex-col">
              {others.map((menu, index) => (
                ...
                >
                  <p
                    className={`w-full flex items-center space-x-3 py-3 px-4 ${
                      active === menu.title && sidebarOpen
                        ? "bg-white text-black"
                        : ""
                    }`}
