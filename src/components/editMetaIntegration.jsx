...
const EditMetaIntegration = ({ showEditCard, tables }) => {
  ...
  const handleDelete = async () => {
    try {
      ...
      if (response.data.success) {
        toast.success(response.data.message);
        setMetaIntegrationList((prev) =>
          prev.filter((table) => table._id !== tableId)
        );
        setOpenModal(false);
      }
    } catch (error) {
            ...
            >
              <div className="bg-white rounded-large border border-secondary shadow-md  pr-0">
                <form
                  ...
                  className="px-normal pt-normal flex flex-col gap-2"
                >
                ...
                <div className="space-y-3 overflow-auto min-h-[calc(100vh-24.5rem)] max-h-[calc(100vh-24.5rem)] my-normal">
                  {metaIntegrationList.map((table, index) => (
                    <div
                      key={index}
                      className={`mx-normal bg-primary rounded-large border border-[#EBF0ED]`}
                    >
                      <div className="flex justify-between items-center p-normal">
                        <div className="space-y-2">
                          <h2 className="text-small font-medium text-neutral-900">
                            {table.tableName}
                          </h2>
                          <p className="text-labelSize text-neutral-900">
                            {table.description}
                          </p>
                        </div>
                        <span
