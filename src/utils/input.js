...
export const TextInput = ({ input, register, errors, disabled }) => {
  ...
  return (
    <div className="flex flex-col gap-1 md:gap-2 justify-start w-full relative">
      <label
        htmlFor={input.name}
        className="text-labelSize text-gray font-medium mx-2"
      >
        {input.label}
      </label>
...
export const TextInputWithoutLabel = ({
  ...
  return (
    <div
      className={`${className} flex flex-col gap-1 md:gap-2 justify-start w-full relative`}
    >
      <div className="relative w-full">
        <input
          type={
          ...
          className={`${
            (input.name === "query" || input.name === "resultType") &&
            "cursor-not-allowed"
          } h-[40px] w-full border border-[#EBF0ED] rounded-large bg-primary px-3 pr-10 focus:border-secondary focus:outline-none text-labelSize md:h-[45px]`}
        />
        {(input.name === "password" || input.name === "confirmPassword") && (
          <span
            onClick={() =>
